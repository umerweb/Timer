...
import { COLORS, TYPOGRAPHY, TIMER_TEMPLATES, TIMEZONES, DEFAULT_TIMER_CFG } from "../theme";
...
<table cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:600px;margin:0 auto;">
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(await res.text());
      const saved = await res.json();
      console.log("[EditorScreen] PUT response:", JSON.stringify(saved, null, 2));
      setTimerName(saved.name);
      setSaveRev((r) => r + 1);
      setSaveStatus("saved");
      setTimeout(() => { setSaveStatus(""); onSaved(saved); }, 1200);
    } catch (err) {
      console.error("[EditorScreen] Update failed:", err);
      setSaveStatus("error");
      setTimeout(() => setSaveStatus(""), 1200);
    }
  };
          ...
          <div className="flex-1 overflow-y-auto p-[18px]">
            ...
            {tab === "design" && (
              <div>
                ...
                <Field label="Font">
                  <select value={cfg.font || "Orbitron"} onChange={(e) => sc("font", e.target.value)}
                ...
                <Field label={`Font Size: ${cfg.fontSize}px`} mb="mb-3.5">
                  <input type="range" min={18} max={48} value={cfg.fontSize || 36} onChange={(e) => sc("fontSize", +e.target.value)} />
                </Field>
                    ...
                    {serverOnline && (
                      <Card>
                        ...
                        <div className="bg-[#111] rounded-lg p-3 text-center mb-2">
                          <img key={saveRev} src={shortGifUrl} alt="Timer GIF"
                  ...
                  : <>
                    ...
                    <Card>
                      ...
                      <div className="flex gap-2 items-center">
                        <input readOnly value={cleanEmbedUrl}
