...
/* ─── UPDATED: High-resolution GIF rendering ─────────── */
function renderGifBuffer(p) {
  return new Promise((resolve, reject) => {
    const SCALE = 1.5; // 🔹 higher resolution
    ...
    encoder.setQuality(10); // 🔹 better color quality
    ...
    const total =
      base.days * 86400 + base.hours * 3600 + base.minutes * 60 + base.seconds;
...
function drawFrame(ctx, W, H, timeObj, p) {
  ...
  const gap = Math.max(
    6,
    Math.min(16, Math.floor(maxInner / (units.length * 10))),
  );
  let boxW = p.fontSize * 2.4;
  const maxBoxW = Math.floor(
    (maxInner - gap * (units.length - 1)) / units.length,
  );
  ...
  const sy = Math.max(
    titleH + 8,
    (H - (titleH + innerBoxH + ls + 6)) / 2 + titleH,
  );

  const br =
    style.boxRadius !== undefined
      ? style.boxRadius
  ...
  units.forEach(({ lbl, val }, i) => {
    ...
    if (i < units.length - 1) {
      ...
      ctx.fillText(
        ":",
        x + boxW + gap / 2,
        y + innerBoxH / 2 - Math.round(fs * 0.08),
      );
      ctx.globalAlpha = 1;
    }
  });
...
router.get("/preview", (req, res) => {
  try {
    ...
    drawFrame(
      ...
      p.width,
      p.height,
      calcTime(p.target, p.timezone, p.mode, p.countUp, p.egHours),
      p,
    );
